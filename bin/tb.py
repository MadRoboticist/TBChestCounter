...
# ------------------------------------------------------------------------------------------------------
    def __init__(self, player_file, config):
        ...
        if self.config.clan not in self.player_set:
            self.player_set_changed = True
            self.player_set.add(self.config.clan)
            print("added player label for clan")
...
# -------------------------------------------------------------------------------
    def validate(self, player, line):
        ...
        if player not in self.player_set:
            ...
            print("Attempting a fuzzy match")
            # attempt a fuzzy match
            best_score = 0
            best_string = ""

            for tmp_player in self.player_set:
                #print("len(temp_player)=%d, len(player)=%d",len(tmp_player),len(player))        
                tmp_score = SM(None, player, tmp_player).ratio()
                
                if tmp_score > best_score and len(tmp_player)==len(player):
...
# ------------------------------------------------------------------------------------------------------    
    def __init__(self, score_file):
        ...
        self.convert_chest_to_source = {"bank","ban","ba","clash for the throne tournament","rise of the ancients event"}
...
# ------------------------------------------------------------------------------------------------------
    def calculate(self, source, chest, player, line):
        ...
        if source.lower() in self.convert_chest_to_source:
            ...
            elif tmp_source.find("rise of the ancients") > -1:
                #print("*********************")
                chest = tmp_source
                #print("Source: "+source)
                if source.find("Ancients") > -1:
                    #print("####################")
                    source = "Ancients' Chest"
                elif source.find("Quick") > -1: 
                    source = "Quick March Chest"
            else:
                chest = 'Bank'
            ...
            with open(file) as sp:
                ...
                while True:
                    ...
                    if int(score) > 0:
                        ...
                        self.player_summary[self.config.clan][1] = self.player_summary[self.config.clan][1] + 1 # increase clan chest number count by 1
...
# ------------------------------------------------------------------------------------------------------
    def run_citadel_summary(self):
        ...
        #print(citadel_summary_sorted)

        for player in citadel_summary_sorted:
            
            if (self.skip_empty and citadel_summary_sorted[player][7] > 0) or not self.skip_empty: # skip lines with 0 count
                #print(player+" "+str(citadel_summary_sorted[player][7]))
                opf.writelines("{},{},{},{},{},{},{},{},{}\n".format( player, 
